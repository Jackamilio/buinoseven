...
#define dmaGetChannel (DMAC->INTPEND.reg & DMAC_INTPEND_ID_Msk)
#define dmaSetChannel( channel ) DMAC->CHID.reg = DMAC_CHID_ID( channel )
...
struct dmaHandle {
	...
	volatile void (*oncompletedcallback)(void);
};
volatile dmaHandle dmaHandles[dmaMaxChannels];
...
void DMAC_Handler() {
	__disable_irq();
	uint32_t channel = dmaGetChannel;
	if (channel > dmaMaxChannels) {
		__enable_irq();
		return;
	}
	...
	volatile dmaHandle& h = dmaHandles[channel];
	h.transfering = false;
	if (h.oncompletedcallback) {
		h.oncompletedcallback();
	}
...
void dmaSetCompletedCallback(uint32_t channel, void (*callback)(void)) {
	if (channel < dmaMaxChannels) {
		dmaHandles[channel].oncompletedcallback = (volatile void (*)(void))callback;
	}
}
...
//	DMAC->CTRL.reg = DMAC_CTRL_SWRST;
	...
	//dmadone = 1;
	memset((void*)dmaHandles,0,sizeof(dmaHandle)*dmaMaxChannels);
}
/*
...
void memcpy32( uint32_t channel, void *dst, const void *src, size_t n) {
	...
	//dmadone = 0;
	dmaHandles[channel].transfering = true;
...
//	dmaState[ chnltx ] = 1;
	//dmadone = 0;
	dmaHandles[chnltx].transfering = true;
...
void dmaWait( uint32_t channel ){
	//while( !dmadone );
	while(dmaHandles[channel].transfering);
